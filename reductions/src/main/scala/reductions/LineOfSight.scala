...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    def iter(i: Int, max: Float): Unit = if (i < input.length) {
      val elem = Math.max(input(i) / i, max)
      output(i) = elem
      iter(i + 1, elem)
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    @scala.annotation.tailrec
    def iter(i: Int, acc: Float): Float =
      if (i < until) iter(i + 1, Math.max(input(i) / i, acc))
      else acc
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    @scala.annotation.tailrec
    def iter(i: Int, max: Float): Unit = if (i < until) {
      val elem = Math.max(input(i) / i, max)
      output(i) = elem
      iter(i + 1, elem)
    }

    iter(from, startingAngle)
  }
